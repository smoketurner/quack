...
| `oidc` | server sign-in through an OpenID Connect issuer (`SignIn`), verification of the issuer's access tokens presented as bearers (`SignIn::verify_bearer`), signed-in users' tokens sealed in `control.db` (`UserTokens`), and each person's own token for session renewal and on-behalf-of exchanges (`PersonTokens`) | 10.2, 12 |
