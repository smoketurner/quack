...
| `storage::writer` | the workspace's one writer connection as an actor: a thread of its own runs the closures sent to it, interactive before background; `run` (async) and `call` (blocking, off the runtime) | 4.1, 7.4 |
