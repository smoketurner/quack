...
pub(crate) async fn run(
    ...
    match action {
        ...
        | OntologyAction::Reject { .. }) => {
            let text = db
                .with(move |db| Ok(rendered(|buf| run_review(db, review, buf))))
                .await?;
            out.write_all(&text.map_err(anyhow::Error::msg)?)?;
        }
        manage => {
            let text = db
                .with(move |db| Ok(rendered(|buf| run_manage(db, manage, buf))))
                .await?;
            out.write_all(&text.map_err(anyhow::Error::msg)?)?;
        }
    }
...
async fn propose(
    ...
    let current = db.with(store::current).await?;
    ...
    let mut proposals = db
        .with(move |db| propose_from_tables(db, known.as_ref(), &evidence))
        .await?;
    if let Some(pass) = &args.documents {
        ...
        let cost = db
            .with(move |db| documents::estimate(db, &estimating))
            .await?;
        writeln!(
            out,
    ...
    if proposals.is_empty() {
        ...
        let run = proposals.clone();
        db.with(move |db| candidates::store_run(db, &run)).await?;
...
async fn run_documents(
    ...
    let sample = db
        .with(move |db| documents::sample_chunks(db, count))
        .await?;
