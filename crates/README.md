...
| `quack` | The one binary: interactive terminal session (no arguments), `-p` print mode, `-q` SQL, `ingest`, `auth`, and the server admin commands (`user`, `token`, `member`, `audit`) | `quack-core`, `clap`, `ratatui`, `ratatui-textarea`, `crossterm`, `anyhow`, `tokio`, `mimalloc` |
