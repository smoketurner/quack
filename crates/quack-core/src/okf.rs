...
/// table, class, relation, document, and graph node, and `log.md` from the
/// ontology versions and the audit detail.
///
/// # Errors
///
...
pub fn export(db: &WorkspaceDb, workspace_name: &str) -> Result<Bundle> {
    ...
    for row in audit::list(db, 500)? {
        writeln!(
            ...
            row.user_id
                .as_deref()
                .map_or(String::new(), |u| format!(" by {u}")),
            row.detail
                .as_ref()
...
fn export_entities(
    ...
    let path_of =
        |node: &graph::Node| format!("entities/{}/{}.md", slug(&node.class_id), slug(&node.label));
    let mut seen: BTreeSet<String> = BTreeSet::new();
    for node in &nodes {
        let mut path = path_of(node);
        ...
        let text = entity_file(node, &edges, &by_id, &provenance, &filename_of)?;
        bundle.push(path.clone(), text);
...
fn entity_file(
    ...
    if !mine.is_empty() {
        ...
        for edge in &mine {
            ...
            if let Some(target) = by_id.get(other) {
                writeln!(
                    text,
                    "- {}: [{}](../{}/{}.md)",
                    edge.relation_id,
                    target.label,
                    slug(&target.class_id),
                    slug(&target.label)
                )?;
            }
        }
