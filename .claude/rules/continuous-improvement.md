...
- Workspace connection confinement: a read-classified `SELECT` over `read_text`,
  `read_csv`, or `FROM 'file'` outside the workspace directory fails, `SET` fails after
  open, and CSV ingestion from `files/` still works
