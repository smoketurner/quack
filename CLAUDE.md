...
entity that exists only in mapped table rows says so instead of returning nothing. Ollama
embedding requests go through `llm::OllamaEmbedder`, not rig's client, so they carry
`keep_alive` and a chunk-sized `num_ctx`.
